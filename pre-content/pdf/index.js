...
function _handlePreContent(inputDir, outputDir, tocPDF, pdfInfo) {
  ...
  return Q().then(function () {
    return fileHelper.listFilesByExtension(extrasDir, '.pdf');
    ...
    return fileHelper.listFilesByExtension(inBookExtrasDir, '.pdf');
  }).then(function (extras) {
    if (extras.length) {
    ...
    return fileHelper.listFilesByExtension(introDir, '.md');
...
function _renderTocPDF(outputDir, pdfInfo) {
...
function _headerAndFooter(pdfInfo) {
  return Q().then(function () {
...
function addPreContent() {
  ...
  return Q()
    .then(function () {
      ...
      return _renderTocPDF(outputDir, pdfInfo);
    }).then(function (tocPDF) {
      return tocHandler.findLinkPositions(tocPDF, pdfInfo);
    }).then(function (tocPDF) {
      return _handlePreContent(inputDir, outputDir, tocPDF, pdfInfo);
    }).then(function () {
      return _headerAndFooter(pdfInfo);
    }).then(function () {
